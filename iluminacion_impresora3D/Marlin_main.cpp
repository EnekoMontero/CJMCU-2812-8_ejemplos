...
void setup() {
  ...
  #ifdef STAT_LED_WS
    FastLED.addLeds<NEOPIXEL, STAT_LED_WS>(leds, NUM_LEDS);
  #endif
...
void prepare_move_to_destination() {
  ...
  void handle_status_leds(void) {
    ...
    if (ELAPSED(millis(), next_status_led_update_ms)) {
      next_status_led_update_ms += 100; // Update every 0.1s
      ExtTemp = thermalManager.degHotend(0); //en versiones nuevas thermalManager.degHotend(0);
      ExtTarg = thermalManager.degTargetHotend(0); //en versiones nuevas thermalManager.degTargetHotend(0);
      //MODO ESPERA
      if(ExtTarg == 0 && ExtTemp <= 30)
      {
        if (flagPaso)
        {
          paso++;
          leds[paso] = CRGB::Red;  
          leds[paso-1] = CRGB::Black;
          if (paso == NUM_LEDS - 1) flagPaso = false;
        }
        else
        {
          paso--;
          leds[paso] = CRGB::Red;  
          leds[paso+1] = CRGB::Black;
          if(paso==0) flagPaso = true;
        }
      }
      //MODO ESPERA
      //MODO IMPRIMIR (apagado para que no consuman)
      ...
      //MODO IMPRIMIR
      //MODO CALENTAR
      else if(ExtTarg != 0 && ExtTemp <= 30)
      {
        for (byte i=0; i<NUM_LEDS; i++)
          leds[i] = CRGB::Blue;
      }
      //Cuando el hotend casi alcanza la temperatura de calentamiento que sea rojo
      else if(ExtTarg!=0 && ExtTemp >= ExtTarg-10)
      {
        for (byte i=0; i<NUM_LEDS; i++)
          leds[i] = CRGB::Red;
      }
      /* Calentamiento hotend
         fading de azul a rojo en función de la temperatura
      */
      else if(ExtTarg != 0)
      {
        Rval = map(ExtTemp,30,ExtTarg-10,0,255);
        Bval = 255 - Rval;
        for (byte i=0; i<NUM_LEDS; i++)
        {
          leds[i].r = Rval;
      ...
      //MODO CALENTAR
      //MODO ENFRIAR
      else if (ExtTarg == 0 && ExtTemp > 200)
      {
        for (byte i=0; i< NUM_LEDS-1; i+=2)
        {
          leds[i] = CRGB::Red;
          leds[i+1] = CRGB::Green;
        }
